...
HexExt = Vector2(Inst.gameObject.GetComponent.<Collider>().bounds.extents.x, Inst.gameObject.GetComponent.<Collider>().bounds.extents.z);
HexSize = Vector2(Inst.gameObject.GetComponent.<Collider>().bounds.size.x, Inst.gameObject.GetComponent.<Collider>().bounds.size.z);
Destroy(Inst);
...
var ray1 = Cam1.ScreenPointToRay (MousePos);
	if (Physics.Raycast (ray1, MouseHex, 100)) {
		...
		if(Input.GetMouseButtonDown(0)){
			if(SelectedHex != null){
				SelectedHex.GetComponent.<Renderer>().material.color = Color.white;
				SelectedHex.GetComponent(FX_HexInfo).IsSelected = false;
			}
			SelectedHex = MouseHex.collider.gameObject;
			SelectedHex.GetComponent.<Renderer>().material.color = Color.blue;
